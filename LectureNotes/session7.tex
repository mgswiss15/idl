\section{Generalization \& Regularization}\label{sec:regularization}
\slideref{Session 7}
...
\centering
\tikzsetnextfilename{overfit_underfit}
\resizebox{0.36\textwidth}{!}{\input{\CommonPath/tikz/overfit_underfit.tex}}
...
\begin{wrapfigure}{r}{0.38\textwidth}
\centering
\tikzsetnextfilename{flat_sharp_minima}
\resizebox{0.36\textwidth}{!}{\input{\CommonPath/tikz/flat_sharp_minima.tex}}
...
\centering
\tikzsetnextfilename{double_descent}
\resizebox{0.40\textwidth}{!}{\input{\CommonPath/tikz/double_descent.tex}}
...
\begin{wrapfigure}{r}{0.38\textwidth}
\centering
\tikzsetnextfilename{dropout}
\resizebox{0.36\textwidth}{!}{\input{\CommonPath/tikz/dropout.tex}}
...
Typical values of $p$ range from 0.1 to 0.5, tuned on the validation set. Dropout works best when applied to \textbf{fully connected layers}. In other architectures - convolutional networks (CNNs), recurrent networks (RNNs), transformers - the standard formulation is less effective or requires adaptation. Dropout also interacts with batch normalisation in a non-trivial way; this is addressed in section~\ref{sec:batchnorm}.
...
\begin{wrapfigure}{r}{0.38\textwidth}
\centering
\tikzsetnextfilename{early_stopping}
\resizebox{0.36\textwidth}{!}{\input{\CommonPath/tikz/early_stopping.tex}}
...
For standard MLPs and CNNs, \textbf{Xavier and He initialisation} discussion in section~\ref{sec:initialization} and \textbf{batch normalisation} of section~\ref{sec:batchnorm} largely solve the vanishing and exploding gradient problem in practice. Initialisation keeps the Jacobian singular values close to 1 at the start of training; batch normalisation stabilises the forward pass distributions throughout training, which indirectly stabilises the backward pass. Gradient instability is therefore rarely a practical concern for these architectures.

The situation is more severe in recurrent networks and transformers, where gradients must propagate through long sequences of dependencies. Here, initialisation and normalisation alone are not sufficient and dedicated solutions are needed. \textbf{Gradient clipping} rescales the gradient if its norm exceeds a threshold $\tau$ before the update step, preventing any single step from being catastrophically large. In PyTorch: \texttt{torch.nn.utils.clip\_grad\_norm\_(model.parameters(), max\_norm=1.0)}, called between \texttt{loss.backward()} and \texttt{optimizer.step()}. \textbf{Spectral normalisation} constrains the spectral norm of weight matrices, bounding the Jacobian singular values by construction. Both are standard tools in recurrent and attention-based architectures, which we will study in later sessions.
...
In practice, the dying ReLU problem is less severe than it first appears. A neuron dies only if its pre-activation is negative for \emph{all} training examples simultaneously - in a diverse dataset, pre-activations vary across examples and the average gradient over a mini-batch is nonzero. He initialisation further reduces the risk by keeping pre-activations in a healthy range at the start of training. Batch normalisation helps too: by keeping the pre-activation distribution centred at zero throughout training, it prevents the drift toward predominantly negative values that would increase the risk of neurons dying. It also guarantees that approximately half the pre-activations remain positive at all times - and for sufficiently wide networks, this is always enough active neurons to represent the necessary functions. ReLU therefore remains the recommended baseline: simple, computationally cheap, and well-protected by the standard training recipe of He initialisation and batch normalisation.
\begin{wrapfigure}{r}{0.38\textwidth}
\centering
\tikzsetnextfilename{activation_functions}
\resizebox{0.36\textwidth}{!}{\input{\CommonPath/tikz/activation_functions.tex}}
\caption{ReLU, LeakyReLU ($\alpha=0.1$), and ELU. LeakyReLU introduces a small negative slope; ELU uses a smooth exponential curve. Both maintain a nonzero gradient for negative pre-activations.}
...
A recurring observation throughout this lecture is that randomness appears at every stage of deep learning training - not as a nuisance to be eliminated but as a deliberate feature that contributes to generalisation. Table~\ref{tab:randomness} summarises the main sources of randomness and their effects.
