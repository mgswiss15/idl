...
\centering
\vspace{-0.5em}
\tikzsetnextfilename{loss_landscape_3d}
\resizebox{0.43\textwidth}{!}{\input{\CommonPath/tikz/loss_landscape_3d.tex}}
...
\begin{wrapfigure}{r}{0.45\textwidth}
\centering
\tikzsetnextfilename{loss_landscape_features}
\resizebox{0.43\textwidth}{!}{\input{\CommonPath/tikz/loss_landscape_features.tex}}
...
\centering
\tikzsetnextfilename{gd_descent_steps}
\resizebox{0.40\textwidth}{!}{\input{\CommonPath/tikz/gd_descent_steps.tex}}
...
\slideref{Session 5: Why Feature Scale Matters, Input normalisation, Initialization: The Symmetry Problem, Initialization: Vanishing and Exploding Signals, Xavier and He Initialization}
...
\subsubsection{Weight Initialisation}\label{sec:initialization}
...
Random initialisation is necessary but not sufficient — the scale of the random weights matters critically. To see why, consider an $L$-layer network with linear activations and weights drawn independently with $\mathbb{E}[\theta] = 0$ and $\mathrm{Var}[\theta] = \sigma^2$, with $n_{\text{in}}$ inputs per layer. The pre-activation at each layer is a weighted sum of $n_{\text{in}}$ independent inputs:
\begin{equation}
    z_j^{(l)} = \sum_{i=1}^{n_{\text{in}}} \theta_{ji}^{(l)}\, h_i^{(l-1)}.
\end{equation}
Since the weights and inputs are independent and zero-mean, the variance of this sum is the sum of the variances of the individual terms. Each term $\theta_{ji} h_i$ has variance $\mathrm{Var}[\theta]\cdot\mathrm{Var}[h] = \sigma^2\cdot\mathrm{Var}[h]$, and there are $n_{\text{in}}$ such terms, giving:
\begin{equation}
...
If $n_{\text{in}}\,\sigma^2 < 1$, the variance shrinks at every layer. By the time the signal reaches the output in a deep network, it is effectively zero — activations are all near zero, and the network cannot distinguish between different inputs. More critically, backpropagation propagates gradients \emph{backward} through the same weights, so the gradient variance shrinks by the same factor at each layer. The gradients reaching the early layers are vanishingly small: those layers receive almost no learning signal and their weights barely move during training, regardless of how many gradient steps are taken. The network learns only from its final layers, and its early feature detectors remain random.
...
\end{equation}
This is \textbf{He} (or Kaiming) initialisation.
...
Mini-batch gradients are noisy: each batch produces a slightly different gradient estimate, causing the parameter updates to fluctuate. In a narrow valley — where the loss is steep across the valley and flat along it — this noise produces rapid oscillations across the valley while progress along the valley is slow. \textbf{Momentum} addresses this by accumulating a velocity vector $\vm_t$, defined as an exponential moving average of past gradients:
\begin{align}
...
\begin{wrapfigure}{r}{0.42\textwidth}
\centering
\tikzsetnextfilename{momentum_vs_sgd}
\resizebox{0.40\textwidth}{!}{\input{\CommonPath/tikz/momentum_vs_sgd.tex}}
\caption{SGD oscillates across a narrow valley. Momentum follows a smoother path.}
\label{fig:momentum}
\end{wrapfigure}
This raises an apparent tension with our earlier argument that mini-batch noise is beneficial for landscape exploration. The resolution is that momentum does not remove the stochasticity — it smooths the gradient \emph{direction} across recent batch updates, but each update still uses a different random batch. The randomness that drives landscape exploration comes from sampling different data points each time; that remains intact. What momentum removes is the erratic variation in step direction caused by the high variance of individual batch gradients — the kind of oscillation that makes the optimiser zigzag across a valley without making progress. The two ideas are therefore complementary: stochastic batching provides the exploratory randomness, and momentum channels that into more directed, efficient steps.
...
Momentum smooths the update direction but uses the same effective learning rate for every parameter. In practice, different parameters may have very different gradient magnitudes — some are sensitive (large gradients) and others are nearly flat (small gradients) — and a single $\eta$ is simultaneously too large for the former and too small for the latter. This problem persists even after input normalisation: normalisation fixes the scale mismatch at the input, but does not control what happens inside the network. As weights evolve during training, different parameters in different layers develop very different gradient magnitudes, and this imbalance changes continuously.

\textbf{RMSProp} addresses this by tracking, for each parameter separately, how large its gradients have been recently. The natural measure is the squared gradient, which removes the sign so that both positive and negative gradients contribute equally to the magnitude estimate. RMSProp maintains an exponential moving average of these squared gradients:
\begin{equation}
    \vv_t = \gamma\, \vv_{t-1} + (1-\gamma)\, \bigl(\nabla_{\pvec} \hat{\loss}_t\bigr)^2,
\end{equation}
with $\gamma \approx 0.99$. A parameter with consistently large gradients accumulates a large $\vv_t$; one with small gradients accumulates a small $\vv_t$. The update divides by $\sqrt{\vv_t}$ to normalise the gradient by its recent typical magnitude:
\begin{equation}
...
The key distinction from momentum is that RMSProp accumulates squared gradients and carries no directional information. Momentum is about smoothing the \emph{direction} of updates; RMSProp is about adapting their \emph{magnitude}. The two are complementary, which motivates combining them.

\subsubsection{Adam}

\textbf{Adam} \parencite{kingma2015adam} (Adaptive Moment Estimation) combines momentum and RMSProp into a single update rule, maintaining both a smoothed gradient direction and a per-parameter magnitude estimate simultaneously:
\begin{align}
...
\end{equation}
When $t$ is small, $\beta^t$ is close to 1 and the correction is large, inflating the early underestimates back to a reasonable scale. As $t$ grows, $\beta^t \to 0$, the corrections approach 1, and the bias correction quietly switches off. The parameter update is then:
\begin{equation}
...
A fixed learning rate is a compromise: a large $\eta$ enables fast initial progress but overshoots near a minimum; a small $\eta$ allows fine-grained convergence but is impractically slow early on. \textbf{Learning rate schedules} vary $\eta$ over the course of training to get the best of both regimes. Figure~\ref{fig:lr_schedules} illustrates the three schedules discussed below.

\begin{wrapfigure}{r}{0.45\textwidth}
\centering
\vspace{-0.5em}
\tikzsetnextfilename{lr_schedules}
\resizebox{0.43\textwidth}{!}{\input{\CommonPath/tikz/lr_schedules.tex}}
\caption{Three learning rate schedules.}
...
\end{equation}
The staircase pattern visible in Figure~\ref{fig:lr_schedules} is characteristic: $\eta$ is held constant for $k$ steps then drops abruptly. This is simple and interpretable, but the sudden drops can cause instability and the flat regions between drops waste the opportunity for finer progress as the optimiser approaches a minimum.
...
\end{equation}
The warmup is motivated by the state of the network at the very start of training: parameters are randomly initialised and far from any good solution, so gradients can be large and unreliable. Starting with a small $\eta$ and ramping it up gradually prevents these early noisy gradients from causing large destabilising updates before the optimiser has settled into a reasonable region of the loss landscape.
