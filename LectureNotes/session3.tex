...
\end{equation}
Real networks used in practice have millions or billions of parameters. Finding good values for all of them jointly from data is the central challenge of deep learning. Figure~\ref{fig:mlp_dimensions} shows the concrete example with all weight matrix dimensions labelled.

\begin{figure}[h]
    \centering
    \resizebox{0.75\textwidth}{!}{\input{\CommonPath/tikz/mlp_dimensions.tex}}
    \caption{A network with $d=3$ inputs, two hidden layers of width $k_1=k_2=4$, and $m=2$ outputs. Weight matrix dimensions are shown above each layer transition. Total parameters: $16 + 20 + 10 = 46$.}
...
\end{equation}
where $\mathbf{1} \in \R^N$ is a vector of ones that broadcasts the bias across all samples. This is precisely what PyTorch computes when we write \texttt{H = X @ W.T + b}: the batch dimension comes first, and the matrix multiply operates across the feature dimension. The block structure of the batch computation is shown in Figure~\ref{fig:batch_matrix_multiply}.

\begin{figure}[h]
    \centering
    \resizebox{0.8\textwidth}{!}{\input{\CommonPath/tikz/batch_matrix_multiply.tex}}
    \caption{Batch forward pass for layer $l$. The input batch $\mathbf{H}^{(l-1)} \in \R^{N \times k_{l-1}}$ is multiplied by $(\thetamat{l})^\top$ and the bias is broadcast across all $N$ samples. PyTorch follows this convention with batch dimension first.}
...
\begin{figure}[h]
    \centering
    \resizebox{0.5\textwidth}{!}{\input{\CommonPath/tikz/learning_problem.tex}}
...
\begin{figure}[h]
    \centering
    \resizebox{0.5\textwidth}{!}{\input{\CommonPath/tikz/mse_loss.tex}}
...
\begin{figure}[h]
    \centering
    \resizebox{0.5\textwidth}{!}{\input{\CommonPath/tikz/gradient_descent_2d.tex}}
